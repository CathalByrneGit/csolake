...
This vignette explains the core concepts behind `csolake` - what data lakes are, how hive partitioning works, and what DuckLake brings to the table. No prior knowledge assumed.
...
`csolake` addresses these issues while keeping the familiar folder-and-permissions model that IT already supports.
...
**For most CSO use cases, start with SQLite.** It's still just a file (familiar, works with IT permissions), but handles multiple users gracefully.
...
Both modes use **file system permissions** - the same model CSO already uses.
