...
This vignette explains how `csolake` works under the hood. It walks through each file in the package, explaining what the code does and why it's designed that way.
...
- `catalog_type` determines which backend to use (default: "duckdb")
- `.db_build_ducklake_dsn()` constructs the correct connection string
- `.db_load_catalog_extensions()` loads sqlite/postgres extensions when needed
- Error handling gives backend-specific hints
- `catalog_type` is stored so `db_status()` can display it

### `db_status()` - Connection Information
...
db_status <- function(verbose = TRUE) {
  ...
  status <- list(
    ...
    catalog_type = .db_get("catalog_type", NA_character_),  # NEW
    ...
  )
  
  if (verbose && status$mode == "ducklake") {
...
The status output now shows the catalog type and its concurrency characteristics.

---

## `R/db_read.R` - Reading Data

### `db_hive_read()` - Read Hive-Partitioned Data
...
db_hive_read <- function(section, dataset, ...) {
  ...
  con <- .db_get_con()
  if (is.null(con)) stop("Not connected...")
  if (.db_get("mode") != "hive") stop("Wrong mode...")
  
  # Build glob path: //CSO-NAS/DataLake/Trade/Imports/**/*.parquet
  base_path <- .db_get("data_path")
  glob_path <- file.path(base_path, section, dataset, "**", "*.parquet")
  
  # Handle ... arguments (extra read_parquet options)
  dots <- list(...)
  if (length(dots) > 0) {
    # Convert R values to SQL: TRUE -> "true", "hello" -> "'hello'"
    opt_sql <- ...
  }
  ...
  query <- paste0(
    ...
    ", hive_partitioning=true",  # Key: enables partition column extraction
    opt_sql,
    ")"
  )
  
  # Permission check: try reading one row
  tryCatch({
    DBI::dbGetQuery(con, paste0("SELECT 1 FROM ", query, " LIMIT 1"))
  }, error = function(e) {
    stop("Unable to read dataset... (helpful error message)")
  })
...
- `**/*.parquet` glob matches all parquet files in all subdirectories
- `hive_partitioning=true` tells DuckDB to extract `year`, `month`, etc. from folder names
- The permission check catches access errors early with a clear message
- Returns a lazy `tbl_duckdb` that you can chain with dplyr verbs

### `db_lake_read()` - Read DuckLake Table

```{r}
db_lake_read <- function(schema = "main", table, version = NULL, timestamp = NULL) {
  ...
  
  # Build AT clause for time travel
  at_sql <- ""
  if (!is.null(version)) {
  ...
  # Full reference: catalog.schema.table AT (VERSION => 5)
  from_sql <- glue::glue("{catalog}.{schema}.{table}{at_sql}")
  
  dplyr::tbl(con, dplyr::sql(from_sql))
}
```

**Key points:**

- Time travel uses DuckDB's `AT` clause
- The catalog prefix ensures we're querying the right DuckLake instance
- Same lazy return pattern as hive read

---

## `R/db_write.R` - Writing Data

### `db_hive_write()` - Write Hive-Partitioned Data
...
db_hive_write <- function(data, section, dataset,
  ...
  # Validate partition columns exist in data
  ...
  # Register data as a temporary DuckDB view
  temp_name <- .db_temp_name()
  duckdb::duckdb_register(con, temp_name, data)
  on.exit(duckdb::duckdb_unregister(con, temp_name))  # Cleanup
  
  # Handle replace_partitions: delete affected folders first
  if (mode == "replace_partitions") {
    # Find unique partition values in the incoming data
    part_vals <- unique(data[partition_by])
    
    # Build folder paths like: Trade/Imports/year=2024/month=01
    to_delete <- vapply(seq_len(nrow(part_vals)), function(i) {
      parts <- vapply(partition_by, function(col) {
        paste0(col, "=", as.character(part_vals[i, col]))
      }, character(1))
      file.path(output_path, paste(parts, collapse = "/"))
    }, character(1))
  ...
  if (mode == "overwrite") opts <- c(opts, "OVERWRITE")
  if (!is.null(partition_by)) {
    opts <- c(opts, glue::glue("PARTITION_BY ({paste(partition_by, collapse = ', ')})"))
  }
  
  # Execute COPY
  sql <- glue::glue("COPY {temp_name} TO {.db_sql_quote(output_path)} ({paste(opts, collapse = ', ')})")
...
- `duckdb::duckdb_register()` makes an R data frame available to DuckDB without copying
- `on.exit()` ensures cleanup even if the function errors
- `replace_partitions` is surgical: only deletes folders that will be rewritten
- DuckDB's `COPY ... TO` handles the actual partitioned writing
- **Note**: `mode = "append"` requires `partition_by` because DuckDB cannot append to a directory without partitioning

### `db_lake_write()` - Write DuckLake Table
...
db_lake_write <- function(data, schema = "main", table,
  ...
  # Register data as temp view
  tmp <- .db_temp_name()
  duckdb::duckdb_register(con, tmp, data)
  on.exit(duckdb::duckdb_unregister(con, tmp))
  
  # Transaction for atomicity
  DBI::dbExecute(con, "BEGIN")
  
  # Set commit metadata (DuckLake-specific)
  if (!is.null(commit_author) || !is.null(commit_message)) {
    DBI::dbExecute(con, glue::glue(
      "CALL ducklake_set_commit_message({.db_sql_quote(catalog)}, {author_val}, {msg_val})"
    ))
  }
